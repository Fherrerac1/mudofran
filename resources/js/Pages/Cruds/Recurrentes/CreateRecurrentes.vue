    <template>
        <BootstrapModal -modal-id="create-recurrentes-Modal" class="factura-recurrente-modal"
            title="Facturas recurrentes" -modal-size="modal-md">
                                    ...
                                    <div class="card mt-3">
                                        <div class="card-header p-0 position-relative mx-3">
                                            ...
                                            <div class="form-group">
                                                <div class="form-group col-12">
                                                    <label for="facturas" class="fw-bold">Seleccionar Factura:</label>
                                                    <MaterialSelect name="factura_id" placeholder="Seleccionar factura"
                                                        id="facturas" v-model="form.factura_id" :is-required="true"
                                                        :options="facturas.map(factura => ({ value: factura.id, text: factura.numFactura }))" />
                                                </div>
                                            </div>
                                        </div>
                                        <div class="py-4 card-body">
                                            <form @submit.prevent="submitForm">
                                                ...
                                                <div class="row g-3">
                                                    <div class="form-group">
                                                        <label class="form-label fw-bold" for="frecuencia">Frecuencia
                                                            del cobro * </label>
                                                        <MaterialSelect id="frecuencia" v-model="form.frecuencia"
                                                            :options="[{
                                                    ...
                                                    <div class="form-group">
                                                        <label class="form-label fw-bold" for="proxima_fecha">Fecha del
                                                            primer cargo</label>
                                                        <input type="text" :value="proximaFechaFormatted"
                                                            id="proxima_fecha" class="form-control border px-3 bg-light"
                                                            disabled />
                                                    </div>
                                                ...
                                                <div class="mt-3">
                                                    <MaterialButton type="submit" id="submitButton" class="w-100"
...
export default {
    ...
    data() {
        return {
            form: useForm({
                factura_id: '',
                frecuencia: 'diaria', // Default to daily
                fechaInicio: '',
                fechaFin: '',
                proxima_fecha: '',
                repeticiones: null,
    ...
    methods: {
        // Method to calculate proxima_fecha based on frecuencia and fechaInicio
        updateProximaFecha() {
        ...
        submitForm() {
            ...
            this.form.post(route('recurrentes.store'), {
                onSuccess: () => {
                    Swal.fire({
                        icon: 'success',
                        title: 'Éxito',
