...
import CobrosModal from "./Relations/CobrosModal.vue";
...
export default {
    ...
    computed: {
        /**
         * Devuelve un array con las facturas filtradas según los selectores de
         *
         * Primero se filtran las facturas que coinciden con los selectores y
         * luego se ordenan por fecha descendente y, en caso de empate, por
         * número de factura descendente.
         *
         * @return {Array}
         */
        filteredFacturas() {
            ...
            return this.facturas
                .filter((factura) => {
                    ...
                    const isDateInRange =
                        Array.isArray(this.dateRange) && this.dateRange.length === 2
                            ? this.isSameOrBetweenDates(factura.fechaInicio, this.dateRange[0], this.dateRange[1])
                            : true;
                ...
                .sort((a, b) => {
                    ...
                    // Luego por número de factura descendente
                    const getNum = (num) => {
        ...
        filteredFacturasRenderKey() {
            return `${this.selected_cliente}-${this.selected_estado}-${this.dateRange}-${this.selected_serie?.map(s => s.value).join(',')}`;
        },
        totalSeleccionado() {
        ...
        confirmDuplicate(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    location.href = route('facturas.duplicate', id);
                }
            });
        },
    },
    ...
    <AuthenticatedLayout :user="user" :title="'Lista de Facturas'">
        <div class="container-fluid overflow-visible">
                ...
                <div class="col d-flex justify-content-between" id="nav">
                    ...
                    <div class="d-flex col justify-content-end gap-3">
                        ...
                        <a href="/facturas/create">
                            <material-button class="float-right btn btm-sm">
                                <i class="fas fa-plus me-2"></i> Agregar Factura
                            </material-button>
                        </a>
                    </div>
                </div>
            ...
            <div class="row mb-4">
                ...
                <!-- serie Select -->
                <div class="form-group col-lg-3 mt-2">
                    <Multiselect id="serie" :multiple="true" v-model="selected_serie" placeholder="Filtrar por Serie"
                        :options="seriesOptions" :close-on-select="false" :clear-on-select="false"
                        :preserve-search="true" label="text" track-by="text"></Multiselect>
                </div>
            ...
            <div id="facturas_div">
                <MaterialTable :key="filteredFacturasRenderKey" title="FACTURAS">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        <tr v-for="factura in filteredFacturas" :key="factura.id" role="button">
                            ...
                            <td class="text-xs font-weight-bold text-start">
                                <strong>{{ factura.estado_text }}</strong>
                            </td>
                            ...
                            <td class="text-xs font-weight-bold text-center">
                                <!-- <a class="me-1" v-if="factura.estado === 0" :href="route('facturas.edit', factura.id)"
                                    title="Editar">
                                    <i class="material-icons">edit</i>
                                </a> -->
                                <a class="me-1" :href="route('facturas.show', factura.id)" title="Ver">
                                    <i class="material-icons">visibility</i>
                                </a>
                                <a class="me-1" href="#" @click="confirmDuplicate(factura.id)" title="Duplicar">
                                    <i class="material-icons">content_copy</i>
                                </a>
                                <a @click="facturaSelected = factura" class="me-1" href="#" data-bs-toggle="modal"
                                    data-bs-target="#cobros_modal" title="Cobros">
        ...
        <CobrosModal :factura="facturaSelected" />
        <LibroModal />
    </AuthenticatedLayout>
</template>
