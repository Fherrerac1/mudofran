<template>
    <!-- cobros Modal -->
    <BootstrapModal -modal-id="cobros_modal" -modal-size="modal-xl" title="Cobros">
        <div v-if="factura.estado === 0 || factura.estado === 5">
            <MaterialButton href="#" data-bs-toggle="modal" data-bs-target="#add_cobro_modal"
                class="d-flex align-items-center ms-auto">
                <i class="material-icons me-2">add</i> Agregar
            </MaterialButton>
        </div>
        <MaterialTable title="Cobros">
            <thead>
                <tr>
            ...
            <tbody>
                <tr v-for="cobro in factura.cobros" :key="cobro.id">
    ...
    <!-- Add Cobro Modal -->
    <BootstrapModal -modal-id="add_cobro_modal" -modal-size="modal-lg" title="Añadir Cobro">
...
export default {
    ...
    mounted() {
        // Set fecha_cobro to today in yyyy-mm-dd format
        ...
        // Set total to totalPendiente
        this.form.total = this.totalPendiente;
    },
    computed: {
        totalCobrado() {
            return Number(this.factura.cobros.reduce((sum, cobro) => sum + cobro.total, 0).toFixed(2));
        },
        totalPendiente() {
            return Number((this.factura.total - this.totalCobrado).toFixed(2));
        },
    },
    methods: {
        ...
        submit() {
            // Validar que no se ingrese un importe mayor al pendiente
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    this.form.post(route('cobro.store', this.factura.id), {
                        onSuccess: () => {
                            Swal.fire({
                                icon: 'success',
                                title: 'Éxito',
