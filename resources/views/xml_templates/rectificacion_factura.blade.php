<sum:RegFactuSistemaFacturacion
    ...
    <sum:RegistroFactura>
        <sum1:RegistroAlta>
            ...
            <sum1:Destinatarios>
                <sum1:IDDestinatario>
                    <sum1:NombreRazon>{{ $factura->cliente_nombre }}</sum1:NombreRazon>
            ...
            <sum1:Desglose>
                <sum1:DetalleDesglose>
                    ...
                    <sum1:CuotaRepercutida>{{ number_format($factura->total_iva, 2, '.', '') }}</sum1:CuotaRepercutida>
                </sum1:DetalleDesglose>
            </sum1:Desglose>
            ...
            <sum1:Encadenamiento>
                <sum1:RegistroAnterior>
                    <sum1:IDEmisorFactura>{{ $factura->tax_id }}</sum1:IDEmisorFactura>
                    <sum1:NumSerieFactura>{{ $ultimaFactura->numFactura ?? 'XXX' }}</sum1:NumSerieFactura>
                    <sum1:FechaExpedicionFactura>
                        {{ $ultimaFactura->fechaInicio ? \Carbon\Carbon::parse($ultimaFactura->fechaInicio)->format('d-m-Y') : 'XXX' }}
                    </sum1:FechaExpedicionFactura>
                    <sum1:Huella>{{ $ultimaFactura->hash ?? 'XXX' }}</sum1:Huella>
                </sum1:RegistroAnterior>
            </sum1:Encadenamiento>
